...
;(function($) {
	...
	var Mapplic = function(element) {
			...
			this.show = function(location) {
				if (location) {
					...
					if (this.tooltiplink) {
						...
						if (location.link) this.link.hide();
						else this.link.hide();
					}
					this.title.text(location.title);
					if (location.description) this.desc.html("Habib");
		...
		function HoverTooltip() {
			...
			// still when I hover the image
			this.show = function(location) {
				if (self.tooltip.location != location) {
					// Show location area when hover
					console.log(location.id);
					this.title.text("Wassa: "+location.id);
					// this.sifat.text("Yup")

					if (this.hovertipdesc) this.desc.html("The Life of people");
